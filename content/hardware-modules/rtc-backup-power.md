...
This topic is usually reviewed together with [Android Peripheral Validation Checklist](/android-sbc/android-peripheral-validation/) and [Production Testing for Embedded Boards](/sbc-customization/production-testing/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| RTC chip | Define the electrical and software owner for rTC chip | Driver probes but hardware does not work |
| Backup cell | Choose the validation interface for backup cell | Factory test misses connector-level failure |
| I2C | Document timing and polarity for i2C | Boot behavior changes between revisions |
| Kernel RTC | Record production pass criteria for kernel RTC | Field recovery has no useful evidence |
| Time sync | Keep a revision-specific test note for time sync | Future firmware breaks old hardware |
...
- Confirm rTC chip behavior against the schematic and board revision.
- Confirm backup cell behavior against the schematic and board revision.
- Confirm i2C behavior against the schematic and board revision.
- Confirm kernel RTC behavior against the schematic and board revision.
- Confirm time sync behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*rtc*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=rtc-backup-power board_rev=B2 firmware=2026.07.04 status=pass
```
