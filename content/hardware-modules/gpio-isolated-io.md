...
This topic is usually reviewed together with [Device Tree Display and GPIO Review](/embedded-linux/device-tree-display/) and [Custom Carrier Board Checklist](/sbc-customization/custom-carrier-board/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| SoC GPIO | Define the electrical and software owner for soC GPIO | Driver probes but hardware does not work |
| Isolation | Choose the validation interface for isolation | Factory test misses connector-level failure |
| Output driver | Document timing and polarity for output driver | Boot behavior changes between revisions |
| libgpiod | Record production pass criteria for libgpiod | Field recovery has no useful evidence |
...
- Confirm soC GPIO behavior against the schematic and board revision.
- Confirm isolation behavior against the schematic and board revision.
- Confirm output driver behavior against the schematic and board revision.
- Confirm libgpiod behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*gpio*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=gpio-isolated-io board_rev=B2 firmware=2026.07.04 status=pass
```
