...
| --- | --- | --- |
| Controller | Define the electrical and software owner for controller | Driver probes but hardware does not work |
| Transceiver | Choose the validation interface for transceiver | Factory test misses connector-level failure |
| Termination | Document timing and polarity for termination | Boot behavior changes between revisions |
| SocketCAN | Record production pass criteria for socketCAN | Field recovery has no useful evidence |
| Error counters | Keep a revision-specific test note for error counters | Future firmware breaks old hardware |
...
- Confirm controller behavior against the schematic and board revision.
...
- Confirm socketCAN behavior against the schematic and board revision.
- Confirm error counters behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*can*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=can-bus-interface board_rev=B2 firmware=2026.07.04 status=pass
```
