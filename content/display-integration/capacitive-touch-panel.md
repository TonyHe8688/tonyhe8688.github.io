...
This topic is usually reviewed together with [Touch Panel Integration on Android](/android-sbc/touch-panel-integration/) and [Touch Calibration and UI Rotation](/hmi-development/touch-calibration-ui-rotation/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.
...
| Interrupt | Document timing and polarity for interrupt | Boot behavior changes between revisions |
| Input events | Record production pass criteria for input events | Field recovery has no useful evidence |
| UI mapping | Keep a revision-specific test note for uI mapping | Future firmware breaks old hardware |
...
- Confirm uI mapping behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*capacitive*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=capacitive-touch-panel board_rev=B2 firmware=2026.07.04 status=pass
```
