...
This topic is usually reviewed together with [Industrial Control UI Structure](/hmi-development/industrial-control-ui/) and [Android OTA and Recovery Strategy](/android-sbc/android-ota-recovery/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Launcher | Define the electrical and software owner for launcher | Driver probes but hardware does not work |
| System UI | Choose the validation interface for system UI | Factory test misses connector-level failure |
| Policy | Document timing and polarity for policy | Boot behavior changes between revisions |
| Watchdog | Record production pass criteria for watchdog | Field recovery has no useful evidence |
| Recovery | Keep a revision-specific test note for recovery | Future firmware breaks old hardware |
...
- Confirm launcher behavior against the schematic and board revision.
- Confirm system UI behavior against the schematic and board revision.
- Confirm policy behavior against the schematic and board revision.
...
```text
TEST_RESULT feature=industrial-android-panel board_rev=B2 firmware=2026.07.04 status=pass
```
