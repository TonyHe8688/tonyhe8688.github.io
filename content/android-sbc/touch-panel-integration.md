...
This topic is usually reviewed together with [Capacitive Touch Panel Integration](/display-integration/capacitive-touch-panel/) and [Touch Calibration and UI Rotation](/hmi-development/touch-calibration-ui-rotation/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| I2C | Define the electrical and software owner for i2C | Driver probes but hardware does not work |
| Interrupt | Choose the validation interface for interrupt | Factory test misses connector-level failure |
| Input events | Document timing and polarity for input events | Boot behavior changes between revisions |
| Rotation | Record production pass criteria for rotation | Field recovery has no useful evidence |
| UI test | Keep a revision-specific test note for uI test | Future firmware breaks old hardware |
...
- Confirm i2C behavior against the schematic and board revision.
- Confirm interrupt behavior against the schematic and board revision.
- Confirm input events behavior against the schematic and board revision.
- Confirm rotation behavior against the schematic and board revision.
- Confirm uI test behavior against the schematic and board revision.
...
```text
TEST_RESULT feature=touch-panel-integration board_rev=B2 firmware=2026.07.04 status=pass
```
