...
This topic is usually reviewed together with [Android Display Debugging](/android-sbc/android-display-debugging/) and [Touch Panel Integration on Android](/android-sbc/touch-panel-integration/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Boot image | Define the electrical and software owner for boot image | Driver probes but hardware does not work |
| Kernel | Choose the validation interface for kernel | Factory test misses connector-level failure |
| HAL | Document timing and polarity for hAL | Boot behavior changes between revisions |
| Display | Record production pass criteria for display | Field recovery has no useful evidence |
| System app | Keep a revision-specific test note for system app | Future firmware breaks old hardware |
...
- Confirm boot image behavior against the schematic and board revision.
- Confirm kernel behavior against the schematic and board revision.
- Confirm hAL behavior against the schematic and board revision.
- Confirm display behavior against the schematic and board revision.
- Confirm system app behavior against the schematic and board revision.
...
```text
TEST_RESULT feature=android-board-bringup board_rev=B2 firmware=2026.07.04 status=pass
```
