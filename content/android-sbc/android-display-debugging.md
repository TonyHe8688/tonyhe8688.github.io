...
This topic is usually reviewed together with [MIPI DSI Display Integration Checklist](/display-integration/mipi-dsi-display/) and [Backlight and Brightness Control](/display-integration/backlight-brightness-control/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Panel driver | Define the electrical and software owner for panel driver | Driver probes but hardware does not work |
| DRM | Choose the validation interface for dRM | Factory test misses connector-level failure |
| HWC | Document timing and polarity for hWC | Boot behavior changes between revisions |
| SurfaceFlinger | Record production pass criteria for surfaceFlinger | Field recovery has no useful evidence |
| App UI | Keep a revision-specific test note for app UI | Future firmware breaks old hardware |
...
- Confirm panel driver behavior against the schematic and board revision.
- Confirm dRM behavior against the schematic and board revision.
- Confirm hWC behavior against the schematic and board revision.
- Confirm surfaceFlinger behavior against the schematic and board revision.
- Confirm app UI behavior against the schematic and board revision.
...
```text
TEST_RESULT feature=android-display-debugging board_rev=B2 firmware=2026.07.04 status=pass
```
