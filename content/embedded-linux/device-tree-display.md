...
This topic is usually reviewed together with [MIPI DSI Display Integration Checklist](/display-integration/mipi-dsi-display/) and [GPIO and Isolated I/O Design Notes](/hardware-modules/gpio-isolated-io/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Schematic | Define the electrical and software owner for schematic | Driver probes but hardware does not work |
| Regulators | Choose the validation interface for regulators | Factory test misses connector-level failure |
| Pinctrl | Document timing and polarity for pinctrl | Boot behavior changes between revisions |
| Display node | Record production pass criteria for display node | Field recovery has no useful evidence |
| Driver probe | Keep a revision-specific test note for driver probe | Future firmware breaks old hardware |
...
- Confirm schematic behavior against the schematic and board revision.
- Confirm regulators behavior against the schematic and board revision.
- Confirm pinctrl behavior against the schematic and board revision.
- Confirm display node behavior against the schematic and board revision.
- Confirm driver probe behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*device*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=device-tree-display board_rev=B2 firmware=2026.07.04 status=pass
```
