...
description: "A checklist for diagnosing bootloader problems during embedded board bring-up."
date: 2026-07-04
avontek_url: ""
---

U-Boot is the first software layer that most board teams can observe and modify. A clean bootloader debug method separates power, strap, DRAM, storage, environment, and kernel handoff problems instead of treating every failure as a single boot issue. This page is written as an engineering note rather than a marketing overview. The goal is to identify the design choices, bring-up checks, validation evidence, and production risks that should be visible before a board or panel product is released.

![U-Boot Debugging for Custom Boards](/images/u-boot-debugging.svg)

## System context

In a real product, u-Boot Debugging for Custom Boards is rarely isolated from the rest of the system. It interacts with power sequencing, connector choices, firmware configuration, device tree or board configuration, factory tests, and the final enclosure. A feature that works on an open bench can still fail in production if the cable length changes, the product warms up, a different peripheral is installed, or the software image handles errors differently.
...
This topic is usually reviewed together with [Rockchip BSP Bring-up Notes](/embedded-linux/rockchip-bsp-notes/) and [Linux Kernel Driver Debug Notes](/embedded-linux/linux-kernel-driver-notes/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Boot ROM | Define the electrical and software owner for boot ROM | Driver probes but hardware does not work |
| SPL | Choose the validation interface for sPL | Factory test misses connector-level failure |
| U-Boot proper | Document timing and polarity for u-Boot proper | Boot behavior changes between revisions |
| Environment | Record production pass criteria for environment | Field recovery has no useful evidence |
| Kernel handoff | Keep a revision-specific test note for kernel handoff | Future firmware breaks old hardware |
...
- Confirm boot ROM behavior against the schematic and board revision.
- Confirm sPL behavior against the schematic and board revision.
- Confirm u-Boot proper behavior against the schematic and board revision.
- Confirm environment behavior against the schematic and board revision.
- Confirm kernel handoff behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*u*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=u-boot-debugging board_rev=B2 firmware=2026.07.04 status=pass
```
