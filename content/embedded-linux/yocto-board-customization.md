...
This topic is usually reviewed together with [Board Revision Tracking and Firmware Compatibility](/sbc-customization/board-revision-firmware/) and [Long-Term Availability and BOM Control](/sbc-customization/bom-lifecycle-control/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| BSP layer | Define the electrical and software owner for bSP layer | Driver probes but hardware does not work |
| Distro layer | Choose the validation interface for distro layer | Factory test misses connector-level failure |
| Machine config | Document timing and polarity for machine config | Boot behavior changes between revisions |
| Recipes | Record production pass criteria for recipes | Field recovery has no useful evidence |
| Image output | Keep a revision-specific test note for image output | Future firmware breaks old hardware |
...
- Confirm bSP layer behavior against the schematic and board revision.
- Confirm distro layer behavior against the schematic and board revision.
- Confirm machine config behavior against the schematic and board revision.
- Confirm recipes behavior against the schematic and board revision.
- Confirm image output behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*yocto*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=yocto-board-customization board_rev=B2 firmware=2026.07.04 status=pass
```
