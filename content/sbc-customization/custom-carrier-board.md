...
This topic is usually reviewed together with [GPIO and Isolated I/O Design Notes](/hardware-modules/gpio-isolated-io/) and [Device Tree Display and GPIO Review](/embedded-linux/device-tree-display/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Power | Define the electrical and software owner for power | Driver probes but hardware does not work |
| Connectors | Choose the validation interface for connectors | Factory test misses connector-level failure |
| Device tree | Document timing and polarity for device tree | Boot behavior changes between revisions |
| Test pads | Record production pass criteria for test pads | Field recovery has no useful evidence |
| Production | Keep a revision-specific test note for production | Future firmware breaks old hardware |
...
- Confirm connectors behavior against the schematic and board revision.
- Confirm device tree behavior against the schematic and board revision.
- Confirm test pads behavior against the schematic and board revision.
- Confirm production behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*custom*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=custom-carrier-board board_rev=B2 firmware=2026.07.04 status=pass
```
