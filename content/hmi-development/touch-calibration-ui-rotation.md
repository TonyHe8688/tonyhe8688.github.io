...
This topic is usually reviewed together with [Capacitive Touch Panel Integration](/display-integration/capacitive-touch-panel/) and [Touch Panel Integration on Android](/android-sbc/touch-panel-integration/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Panel rotation | Define the electrical and software owner for panel rotation | Driver probes but hardware does not work |
| Input transform | Choose the validation interface for input transform | Factory test misses connector-level failure |
| Toolkit | Document timing and polarity for toolkit | Boot behavior changes between revisions |
| Calibration | Record production pass criteria for calibration | Field recovery has no useful evidence |
| QA | Keep a revision-specific test note for qA | Future firmware breaks old hardware |
...
- Confirm panel rotation behavior against the schematic and board revision.
- Confirm input transform behavior against the schematic and board revision.
- Confirm toolkit behavior against the schematic and board revision.
- Confirm calibration behavior against the schematic and board revision.
- Confirm qA behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*touch*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=touch-calibration-ui-rotation board_rev=B2 firmware=2026.07.04 status=pass
```
