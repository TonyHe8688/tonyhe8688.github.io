---
title: "HMI Performance Testing on Low-Power Boards"
description: "A practical test plan for UI latency, redraw cost, boot time, and memory use on embedded boards."
date: 2026-07-04
avontek_url: ""
---
...
This topic is usually reviewed together with [LVGL on Embedded Linux](/hmi-development/lvgl-on-linux/) and [Thermal Design for Enclosed SBCs](/sbc-customization/thermal-design-enclosed-sbc/). Reading the related notes helps connect the board-level decision, software configuration, and production validation path before the design is frozen.

## Design decisions

| Area | Engineering decision | Risk if ignored |
| --- | --- | --- |
| Boot time | Define the electrical and software owner for boot time | Driver probes but hardware does not work |
| Frame time | Choose the validation interface for frame time | Factory test misses connector-level failure |
| CPU | Document timing and polarity for cPU | Boot behavior changes between revisions |
| Memory | Record production pass criteria for memory | Field recovery has no useful evidence |
| Thermal | Keep a revision-specific test note for thermal | Future firmware breaks old hardware |
...
- Confirm boot time behavior against the schematic and board revision.
- Confirm frame time behavior against the schematic and board revision.
- Confirm cPU behavior against the schematic and board revision.
- Confirm memory behavior against the schematic and board revision.
...
```sh
dmesg | tail -80
find /sys -maxdepth 3 -iname "*hmi*" 2>/dev/null
ls -l /dev | head
```
...
```text
TEST_RESULT feature=hmi-performance-testing board_rev=B2 firmware=2026.07.04 status=pass
```
